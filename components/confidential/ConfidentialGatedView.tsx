...
export default function ConfidentialGatedView() {
    ...
    const handleUnlock = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                setIsRequested(false); // Close the modal upon success
            } else {
    ...
    const whatsappNumber = "447397392319";
    ...
    return (
        <section id="confidential" className="relative w-full py-24 bg-[#050505] border-t border-white/5">

            {/* INLINE CONTENT: Teaser OR Unlocked Grid */}
            {!isAuthenticated ? (
                <div className="max-w-3xl mx-auto px-6 flex flex-col items-center text-center">
                    <div className="w-16 h-16 rounded-full bg-white/5 mb-6 flex items-center justify-center border border-white/10">
                        <svg className="w-8 h-8 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                        </svg>
                    </div>
                    <h2 className="text-3xl md:text-5xl font-medium tracking-tight mb-4 text-white">Confidential Collection</h2>
                    ...
                    <AnimatedButton onClick={() => setIsRequested(true)} variant="primary" className="px-8 py-3 text-sm">
                ...
                >
                    <div className="max-w-5xl mx-auto px-6 text-center mb-16">
                        <h2 className="text-3xl md:text-5xl font-medium tracking-tight mb-2 text-white">Confidential Collection</h2>
                        <p className="text-green-500/80 font-mono text-xs uppercase tracking-widest">Access Granted • Secure Session Active</p>
                    </div>
                    <ConfidentialGrid />
                </motion.div>
            )}
            ...
            <AnimatePresence mode="wait">
                {isRequested && !isAuthenticated && !isChatOpen && (
                    <motion.div
                        key="auth-modal"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        ...
                        className="fixed inset-0 z-[100] flex flex-col items-center justify-center px-4 bg-black/80 backdrop-blur-md"
                    >
                        {/* Close button for safety */}
                        <button
                            onClick={() => setIsRequested(false)}
                            className="absolute top-8 right-8 p-3 bg-white/5 hover:bg-white/10 rounded-full text-white/50 hover:text-white transition"
                        >
                        ...
                        <div className="text-center max-w-2xl mb-8 flex flex-col items-center">
                            <h2 className="text-3xl font-medium tracking-tight mb-2 text-white">Unlock Access</h2>
                            <p className="text-sm text-white/50 font-light max-w-md mx-auto">
                                Enter your verified phone number and unique access code to view the secure collection.
                            </p>
                        </div>

                        {/* Interactive Form Box */}
                        <div className="w-full max-w-sm bg-[#111] border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col gap-6">
                            <form onSubmit={handleUnlock} className="flex flex-col gap-4">
                                <input
                                    ...
                                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-center text-base tracking-widest text-white focus:outline-none focus:border-white/40 transition-colors"
                        ...
                        <div className="w-full max-w-md bg-[#0b141a] rounded-2xl overflow-hidden shadow-2xl border border-white/5 flex flex-col h-auto min-h-[450px] max-h-[85vh]">
                            ...
                            <div className="flex-1 bg-[#0b141a] p-4 flex flex-col overflow-y-auto relative bg-[url('https://web.whatsapp.com/img/bg-chat-tile-dark_a4be512e7195b6b733d9110b408f075d.png')] bg-repeat opacity-90">
                                <div className="text-center mb-6 mt-2">
                                    <span className="bg-[#182229] text-white/60 text-xs px-3 py-1.5 rounded-lg shadow-sm">Today</span>
                                </div>
                            ...
                            <div className="bg-[#202c33] p-3 flex items-center gap-2">
                                ...
                                <div className="flex-1 bg-[#2a3942] rounded-lg px-4 py-2 flex items-center">
                                    <input type="text" value={chatMessage} onChange={(e) => setChatMessage(e.target.value)} className="w-full bg-transparent text-[#d1d7db] text-[15px] focus:outline-none placeholder-white/40" placeholder="Type a message" />
                                </div>
                                <button onClick={handleSendWhatsApp} className="p-3 bg-[#00a884] hover:bg-[#00a884]/90 text-white rounded-full transition-colors shrink-0 flex items-center justify-center shadow-md ml-1" aria-label="Send">
